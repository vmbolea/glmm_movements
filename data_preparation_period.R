...
dat <- dat |>
  ...
  dplyr::mutate(
    ...
    dias_desde_inicio = as.numeric(lubridate::date(created_at) - lubridate::date("2024-07-12")), # Días desde la primera fecha
    periodo_num = floor(dias_desde_inicio / 31),      # Número de período de 30 días
    inicio_periodo = lubridate::date("2024-07-12") + lubridate::days(periodo_num * 31), # Fecha de inicio del período
    month = periodo_num,
    context = ifelse(`88_Motivacin_del_des` == "Alimentación", "Feeding", "Defense")
  )  |>
  dplyr::ungroup()
...
write.csv(data_, "data/my_data_period.csv", row.names = FALSE)
